...
function wireUi(){
  document.querySelectorAll(".wallet-tab-list button").forEach(b=>{
  ...
  document.querySelectorAll(".person-switch button").forEach(b=>{
    b.addEventListener("click",()=>{
      document.querySelectorAll(".person-switch button").forEach(x=>x.classList.remove("active"));
      b.classList.add("active");
    });
  });
}
...
function txHTML(t){
  ...
  return `<div class="record-emoji">${emoji}</div><div class="record-main"><b>${t.merchant||""}</b><small>${t.owner==="erol"?"Erol":"Nisu"} · ${t.category||"diğer"} · ${dateText(t.date)}${t.account?` · ${t.account}`:""}</small></div><div class="record-amount">-${money(t.amount)}</div>`;
}
...
async function connectFirebase(){
  try{
    ...
    db=getFirestore(fb);
    walletRef=doc(db,"wallets","shared-ne-wallet");
    fbFns={getDoc,setDoc,onSnapshot};
    ...
    console.warn("Firebase bağlanamadı, arayüz yerel modda çalışıyor:",e);
  }
}

async function persist(){
  localStorage.setItem("ne-wallet-state",JSON.stringify(state));
  if(applyingRemote || !walletRef || !fbFns?.setDoc) return;
  try{ await fbFns.setDoc(walletRef,{...state,updatedAt:new Date().toISOString()}); }
  catch(e){ console.warn("Firebase yazma:",e); }
}

async function load(){
  let base={};
  try{ base=await fetch("./data.json",{cache:"no-store"}).then(r=>r.json()); }catch(e){ console.warn("data.json okunamadı",e); }
  let local=null;
...
async function addExpense(ev){
  ...
  state.transactions.push({id:Date.now(),owner:f.get("owner"),merchant:f.get("merchant"),amount:Number(f.get("amount")),category:f.get("category"),account:f.get("account"),date:f.get("date")});
